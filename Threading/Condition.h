...
protected:
	HANDLE create_anonymous_semaphore(long cur,long max)
	{
		...
		DEDOSAN_INLINE void shift_generations_down()
		{
		...
		DEDOSAN_INLINE void dispose_entry(list_entry& entry)
		{
		...
		DEDOSAN_INLINE HANDLE duplicate_handle(HANDLE source)
		{
...
{
public:
	DEDOSAN_INLINE Condition(Mutex * mutex) : m_nLockCount(0), m_externalMutex(mutex)
	{
	...
	DEDOSAN_INLINE void BeginSynchronized()
	{
		m_externalMutex->Acquire();
		++m_nLockCount;
	}

	DEDOSAN_INLINE void EndSynchronized()
	{
...
{
public:
	DEDOSAN_INLINE Condition(Mutex *m)
	{
		mut=m;
		pthread_cond_init(&cond,NULL);
	}
	DEDOSAN_INLINE ~Condition()
	{
		pthread_cond_destroy(&cond);
	}

	DEDOSAN_INLINE void Signal()
	{
		pthread_cond_signal(&cond);
	}
	DEDOSAN_INLINE void Broadcast()
	{
		pthread_cond_broadcast(&cond);
	}
	DEDOSAN_INLINE void Wait()
	{
		pthread_cond_wait(&cond,&mut->mutex);
	}
	DEDOSAN_INLINE bool Wait(time_t seconds)
	{
		timespec tv;
		tv.tv_nsec = 0;
		tv.tv_sec = seconds;
