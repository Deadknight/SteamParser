...
{
	public:
		registry_value(const std::wstring & name, HKEY base_) : key(0), base(base_)
		{
			make_path(name);
		}
	...
	protected:
		...
		bool open(bool write_access)
		{
			...
			{
				DWORD disposition;
				if(RegCreateKeyEx(base, keyname.c_str(), 0, L"", REG_OPTION_NON_VOLATILE, KEY_WRITE, 0, &key, &disposition) == ERROR_SUCCESS)
					return true;
			}
	...
	private:
		...
		void make_path(const std::wstring & name)
		{
...
{
	public:
		registry_string(const std::wstring & name, HKEY base) : registry_value(name, base)
		{
		}
...
template<>
class registry_string<std::wstring> : public registry_value
{
	public:
		registry_string(const std::wstring & name, HKEY base) : registry_value(name, base)
		{
		...
		const registry_string & operator=(const std::wstring & value)
		{
			if(open(true) == true)
			{
...
{
	public:
		registry_int(const std::wstring & name, HKEY base) : registry_value(name, base)
		{
		}
...
{
	public:
		registry_binary(const std::wstring & name, HKEY base) : registry_value(name, base)
		{
		}
